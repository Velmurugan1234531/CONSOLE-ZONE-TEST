        ...
        if ("geolocation" in navigator) {
            navigator.geolocation.getCurrentPosition(async (position) => {
                ...
                try {
                    ...
                    console.warn(`Geocoding service unavailable: ${error?.message || error}`);
                } finally {
                    setIsLocating(false);
                }
            }, (error: any) => {
        ...
        try {
            ...
            if (!user) {
                ...
                if (demoUser) {
                    ...
                    const idUrl = await uploadFile(idFile, `${parsed.id}/id_card_front`);
                    const idBackUrl = idBackFile ? await uploadFile(idBackFile, `${parsed.id}/id_card_back`) : undefined;
                                    ...
                                    {step === 2 && (
                                        ...
                                        >
                                            ...
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                                    ...
                                                    <div className={`relative group cursor-pointer border-2 border-dashed rounded-2xl p-8 text-center transition-all min-h-[180px] flex flex-col items-center justify-center ${idFile ? 'border-emerald-500 bg-emerald-500/5' : 'border-white/10 hover:border-[#A855F7] hover:bg-white/5'}`}>
                                                        ...
                                                        {idFile ? (
                                                            ...
                                                            <div className="text-center">
                                                                ...
                                                                <p className="text-[10px] font-black uppercase tracking-widest text-gray-300">Sync ID (Front)</p>
                                                            </div>
                                                        )}
                                                    </div>
                                                </div>
                                                ...
                                                {/* ID Card Back Upload */}
                                                <div className="space-y-4">
                                                    <label className="text-xs font-bold text-gray-500 uppercase tracking-widest ml-1 block">Government ID (Back)</label>
                                                    <div className={`relative group cursor-pointer border-2 border-dashed rounded-2xl p-8 text-center transition-all min-h-[180px] flex flex-col items-center justify-center ${idBackFile ? 'border-emerald-500 bg-emerald-500/5' : 'border-white/10 hover:border-[#A855F7] hover:bg-white/5'}`}>
                                                        ...
                                                        {idBackFile ? (
                                                            <div className="text-center">
                                                                <CheckCircle2 className="text-emerald-500 mx-auto mb-2" size={32} />
                                                                <p className="text-xs font-mono text-white truncate max-w-[200px] mx-auto">{idBackFile.name}</p>
                                                            </div>
                                                        ) : (
                                                            <div className="text-center">
                                                                <FileCheck className="text-[#A855F7] mx-auto mb-2 group-hover:scale-110 transition-transform opacity-60" size={32} />
                                                                <p className="text-[10px] font-black uppercase tracking-widest text-gray-300">Sync ID (Back)</p>
                                                            </div>
                                                        )}
                                                    </div>
                                                </div>
                                                ...
                                                <div className="grid grid-cols-2 gap-8">
                                                    ...
                                                    <div className="space-y-4 text-right">
                                                        ...
                                                        <div className="flex justify-end gap-2">
                                                            {idFile && <div className="p-2 bg-emerald-500/10 text-emerald-500 rounded-lg" title="ID Front Sync"><FileCheck size={16} /></div>}
                                                            {idBackFile && <div className="p-2 bg-emerald-500/10 text-emerald-500 rounded-lg" title="ID Back Sync"><FileCheck size={16} className="opacity-60" /></div>}
                                                            {selfieFile && <div className="p-2 bg-emerald-500/10 text-emerald-500 rounded-lg" title="Bio Sync"><User size={16} /></div>}
                                                            {lat && <div className="p-2 bg-[#8B5CF6]/10 text-[#8B5CF6] rounded-lg" title="GPS Lock"><MapPin size={16} /></div>}
                                                        </div>
                                                    </div>
                                                </div>
