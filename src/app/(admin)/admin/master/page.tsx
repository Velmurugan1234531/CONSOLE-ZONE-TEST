...
const LiveTrackingNexus = dynamic(() => import("@/components/admin/master/LiveTrackingNexus").then(mod => mod.LiveTrackingNexus), { ssr: false });
...
export default function MasterControlPage() {
    ...
    useEffect(() => {
        ...
        const load = async () => {
            try {
                // Fetch all data with individual fallback protection and a global layout rescue
                ...
                const timeoutPromise = new Promise((_, reject) =>
                    setTimeout(() => reject(new Error("DASHBOARD_FETCH_TIMEOUT")), 12000)
                );

                const [settings, allDevices, metrics, fleetStats, revenue, kStats] = await Promise.race([fetchPromise, timeoutPromise]) as any;
    ...
    const handleSaveSettings = async () => {
        ...
        try {
            await saveMarketplaceSettings(marketplaceSettings);
        ...
        <div className="min-h-screen bg-[#050505] text-white p-6 lg:p-10 space-y-10 custom-scrollbar overflow-x-hidden">
            ...
            <div className="flex flex-col lg:flex-row gap-8 items-start">
                <nav className="w-full lg:w-72 space-y-2 lg:sticky lg:top-10">
                    ...
                    <NavTab id="identity" icon={Fingerprint} label="KYC VERIFICATION" active={activeTab} onClick={setActiveTab} />
                    <NavTab id="live-tracking" icon={MapPin} label="Live Tracking" active={activeTab} onClick={setActiveTab} />
                </nav>
                ...
                <main className="flex-1 w-full min-h-[600px]">
                    <AnimatePresence mode="wait">
                                ...
                                <CommandNexusCard title="Financial Protocols" subtitle="Global taxes, platform fees, and violation penalties" icon={DollarSign} statusColor="emerald">
                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-6">
                                        <HUDInput
                                            ...
                                            value={(businessSettings?.finance?.taxRate ?? 0).toString()}
                                        ...
                                        <HUDInput
                                            ...
                                            value={(businessSettings?.finance?.platformFee ?? 0).toString()}
                                        ...
                                        <HUDInput
                                            ...
                                            value={(businessSettings?.finance?.lateFeePerDay ?? 0).toString()}
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-12 mt-6">
                                        <div className="space-y-8">
                                            <HUDInput
                                                ...
                                                value={(marketplaceSettings?.tradeInFee ?? 0).toString()}
                            ...
                            <motion.div key="nexus" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-8">
                                <CommandNexusCard title="Integration Matrix" subtitle="Real-time status of critical system pipelines" icon={Activity} statusColor="blue">
                                    <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mt-6">
                                        <NexusCard label="Firestore" status={systemMetrics?.integrations?.firestore} icon={Database} color="emerald" />
                                        <NexusCard label="Razorpay API" status={systemMetrics?.integrations?.razorpay} icon={CreditCard} color="blue" />
                                        <NexusCard label="Auth Matrix" status={systemMetrics?.integrations?.auth || 'active'} icon={Lock} color="purple" />
                        ...
                        {activeTab === 'live-tracking' && (
                            <motion.div key="live-tracking" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
                                <LiveTrackingNexus />
                            </motion.div>
                        )}
                    </AnimatePresence>
