...
import {
    ...
    Database, Trash2, Globe, ClipboardList, type LucideIcon
} from "lucide-react";
import Link from "next/link";
import { getAllRentals, updateRentalStatus, updateRental } from "@/services/admin";
import { exportRentalsToCSV } from "@/services/analytics";
...
const QuickStatsCards = dynamic(() => import("@/components/admin/QuickStatsCards").then(mod => mod.QuickStatsCards), { ssr: false });
const RevenueTrendChart = dynamic(() => import("@/components/admin/RevenueTrendChart").then(mod => mod.RevenueTrendChart), { ssr: false });
...
type ViewType = 'monitor' | 'tracking' | 'ledger' | 'booking' | 'calendar' | 'fleet' | 'engine' | 'requests';

function RentalCountdown({ startDate, endDate }: { startDate: string, endDate: string }) {
    ...
    if (diff > 0) {
        ...
        if (days > 0) timeLabel = `${days}D ${hours}H ${mins}M`;
        else if (hours > 0) timeLabel = `${hours}H ${mins}M ${secs}S`;
        else timeLabel = `${mins}M ${secs}S`;
    }
        ...
        <div className="space-y-3">
            <div className="space-y-1.5">
                <div className="flex justify-between text-[8px] uppercase font-black tracking-widest">
                    ...
                    <span className={progress >= 100 ? "text-red-500" : "text-blue-400"}>
                        {progress.toFixed(1)}%
                    </span>
                </div>
                <div className="h-1 w-full bg-white/5 rounded-full overflow-hidden">
                    <div
                        className={`h-full transition-all duration-1000 ${progress >= 100 ? 'bg-red-500 shadow-[0_0_10px_rgba(239,68,68,0.5)]' : 'bg-[#06B6D4] shadow-[0_0_10px_rgba(6,182,212,0.5)]'}`}
            ...
            <div className="flex justify-between text-[9px] uppercase font-bold tracking-widest">
                ...
                <span className={`${diff <= 0 ? 'text-red-500' : 'text-orange-400'} font-mono text-[8px]`}>
                    {timeLabel}
                </span>
            </div>
        </div>
...
export default function RentalsPage() {
    ...
    const toggleSelectRental = (id: string) => {
    ...
    const handleBulkStatusUpdate = async (newStatus: string) => {
        ...
        try {
            await Promise.all(
                selectedRentals.map(id => updateRentalStatus(id, newStatus))
            );
            alert(`Successfully updated ${selectedRentals.length} rental(s) to ${newStatus}`);
            setSelectedRentals([]);
            fetchRentals();
        } catch (error) {
            alert('Failed to update some rentals');
        } finally {
    ...
    const handleBulkDelete = async () => {
        ...
        const confirmed = confirm(`Are you sure you want to delete ${selectedRentals.length} rental(s)?`);
        if (!confirmed) return;

        setBulkActionLoading(true);
        try {
            // In real implementation, this would call a delete API
            console.log('Deleting rentals:', selectedRentals);
            alert(`Would delete ${selectedRentals.length} rental(s) (not implemented yet)`);
            setSelectedRentals([]);
        } catch (error) {
            alert('Failed to delete rentals');
        } finally {
            setBulkActionLoading(false);
        }
    };
    ...
    return (
        <div className="flex flex-col min-h-screen bg-[#050505] text-white">
            ...
            <header className="sticky top-0 z-20 bg-[#0a0a0a]/95 backdrop-blur-md border-b border-white/10 shadow-2xl">
                        ...
                        <div className="flex items-center gap-6">
                            ...
                            <div className="bg-white/5 p-1 rounded-xl border border-white/10 flex">
                                ...
                                <button
                                    ...
                                    className={`px-4 py-2 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all flex items-center gap-2 ${activeView === 'requests' ? 'bg-[#8B5CF6] text-white shadow-[0_0_15px_rgba(139,92,246,0.3)]' : 'text-gray-500 hover:text-white'}`}
                                >
                                        ...
                                        <div key={rental.id} className="bg-[#0a0a0a] border border-white/10 rounded-2xl p-4 hover:border-[#06B6D4]/50 transition-all group relative overflow-hidden">
                                            ...
                                            <div className="space-y-3 mb-6">
                                                <div className="flex justify-between text-[9px] uppercase font-bold tracking-widest">
                                                    ...
                                                    <span className="text-white truncate max-w-[100px]">{rental.user?.full_name || 'Guest'}</span>
                                                </div>
                                                <RentalCountdown startDate={rental.start_date} endDate={rental.end_date} />
                                            </div>
                            ...
                            <div className="animate-in fade-in duration-500 space-y-4">
                                ...
                                <div className="flex items-center justify-between mb-4">
                                    ...
                                    <button
                                        ...
                                        className="px-4 py-2 bg-emerald-500/10 hover:bg-emerald-500/20 border border-emerald-500/30 text-emerald-500 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all flex items-center gap-2"
                                    >
                                ...
                                <AnimatePresence>
                                    {selectedRentals.length > 0 && (
                                        <motion.div
                                            ...
                                            exit={{ opacity: 0, y: -10 }}
                                            className="bg-[#8B5CF6]/10 border border-[#8B5CF6]/30 rounded-xl p-4 flex items-center justify-between"
                                        >
                                            <div className="flex items-center gap-3">
                                                <div className="bg-[#8B5CF6] text-black rounded-lg px-3 py-1 text-sm font-black">
                                                    {selectedRentals.length}
                                                </div>
                                                <span className="text-sm font-bold text-white">Item{selectedRentals.length > 1 ? 's' : ''} Selected</span>
                                            </div>
                                            <div className="flex items-center gap-2">
                                                <button
                                                    onClick={() => handleBulkStatusUpdate('active')}
                                                    disabled={bulkActionLoading}
                                                    className="px-4 py-2 bg-blue-500/10 hover:bg-blue-500/20 border border-blue-500/30 text-blue-500 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all disabled:opacity-50"
                                                >
                                                    Mark Active
                                                </button>
                                                <button
                                                    ...
                                                    disabled={bulkActionLoading}
                                                    className="px-4 py-2 bg-emerald-500/10 hover:bg-emerald-500/20 border border-emerald-500/30 text-emerald-500 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all disabled:opacity-50"
                                                >
                                                    Mark Completed
                                                </button>
                                                <button
                                                    ...
                                                    disabled={bulkActionLoading}
                                                    className="px-4 py-2 bg-red-500/10 hover:bg-red-500/20 border border-red-500/30 text-red-500 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all disabled:opacity-50 flex items-center gap-2"
                                                >
                                                    <Trash2 size={12} />
                                                    Delete
                                                </button>
                                                <button
                                                    ...
                                                    className="px-3 py-2 bg-white/5 hover:bg-white/10 border border-white/10 text-gray-400 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all"
                                                >
                                                    Clear
                                                </button>
                                            </div>
                                    ...
                                    <table className="w-full text-left">
                                        <thead className="bg-[#050505] text-[9px] font-bold text-gray-500 uppercase tracking-widest border-b border-white/5">
                                            <tr>
                                                ...
                                                <th className="p-5 text-white/40">Deployment Asset</th>
                                        ...
                                        <tbody className="divide-y divide-white/5">
                                            {filteredRentals.map(rental => (
                                                <tr key={rental.id} className={`hover:bg-white/[0.02] transition-colors group ${selectedRentals.includes(rental.id) ? 'bg-[#8B5CF6]/5' : ''}`}>
                                                    <td className="p-5 pl-8">
                                                        <input
                                                            type="checkbox"
                                                            checked={selectedRentals.includes(rental.id)}
                                                            onChange={() => toggleSelectRental(rental.id)}
                                                            className="w-4 h-4 rounded border-white/20 bg-white/5 text-[#8B5CF6] focus:ring-[#8B5CF6] focus:ring-offset-0 cursor-pointer"
                                                        />
                                                    </td>
                                                    <td className="p-5">
            ...
            {activeView === 'requests' && (
                <div className="animate-in fade-in duration-300 bg-[#0a0a0a] rounded-3xl border border-white/10 p-8">
                    <RentalRequestsTable />
                </div>
            )}
        </div>
